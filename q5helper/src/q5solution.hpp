...
T maximum(const NTN<T>& ntn) {
	...
	if (!ntn.children.empty())
		for (auto it : ntn.children){
			T compareMax = maximum(it);
			compareMax > max ? max = compareMax : max;
		}

  return max; //Compiles but is incorrect
}
...
std::string longest_word (const DTN& dtn) {
	std::string answer = dtn.word_to_here;
	if (!dtn.children.empty())
		for (auto it : dtn.children){
			std::string compareStr = longest_word(it.second);
			if (compareStr.size() > answer.size())
				answer = compareStr;
		}

  return answer; //Compiles but is incorrect
}
