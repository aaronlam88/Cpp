...
typedef ics::ArrayPriorityQueue<XRefEntry> XRefPQ;
...
//  returns true.
void print_xref(std::string message, const XRef& xref,
		bool (*has_higher_priority)(const XRefEntry& i, const XRefEntry& j)) {
	std::cout << "\n" << message << std::endl;
	for (XRefEntry kv : XRefPQ(xref.ibegin(), xref.iend(), has_higher_priority))
...
void print_xref(std::string message, XRef& xref,
		bool (*has_higher_priority)(const XRefEntry& i, const XRefEntry& j)) {
	std::cout << "\n" << message << std::endl;
	XRefPQ sorted(xref.ibegin(), xref.iend(), has_higher_priority);
	for (XRefEntry kv : sorted) {
		std::cout << "  " << kv.first << " --> ";
		LineStack lines(kv.second.ibegin(), kv.second.iend());
		int count = 0;
		for (int line : lines)
